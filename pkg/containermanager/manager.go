...
func (m *ContainerManager) Remove(nameOrId string, force bool) error {
	logrus.Infof("Cleaning up container %s", nameOrId)
	...
	if _, ok := m.containers[nameOrId]; !ok {
...
func (m *ContainerManager) PullIfNotPresent(ctx context.Context, image string) error {
	...
	pulled, err := images.Pull(ctx, image, nil)
	if err != nil {
		return err
	}
...
func (m *ContainerManager) Logs(ctx context.Context, nameOrId string) (chan string, chan error, error) {
	...
	go func() {
		out := ""
		errStr := ""
		logrus.Infof("Waiting for logs goroutine")
		for {
			select {
			case o := <-outCh:
				out += o
...
func (m *ContainerManager) TaskT(timeout time.Duration, image string, name string, input interface{}) (interface{}, error) {
	logrus.Infof("Running task '%s'", name)
	ctx, cancel := context.WithTimeout(m.connectionCtx, timeout)
	defer cancel()
	...
	logrus.Infof("Waiting for container %s to stop", id)
	select {
	case err = <-waitErr:
		if err != nil {
			return nil, err
	...
	logrus.Infof("Gathering logs from container %s", id)
	...
	logrus.Infof("Extracting result from stdout:\n\n%s\n\n", result)
...
func (m *ContainerManager) Close(removeImages bool) {
	...
	if removeImages {
		force := true
		for image, _ := range m.images {
			logrus.Infof("Removing image %s", image)
			_, errs := images.Remove(m.connectionCtx, []string{image}, &images.RemoveOptions{Force: &force})
			for _, err := range errs {
				if err != nil {
					logrus.Errorf("Failed to remove image %s: %s", image, err)
				}
			}
		}
	}
